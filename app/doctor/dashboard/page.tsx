...
import { 
  ...
  Plus, Trash2, Pill, Clock, AlertCircle, Phone, CalendarDays, RefreshCw, ArrowRight
} from "lucide-react";
...
type TimelineEvent = {
  date: string;
  title: string;
  type: "diagnosis" | "lab" | "consultation" | "procedure";
  summary: string;
};
...
export default function DoctorDashboard() {
  ...
  const handleRequestAccess = async () => {
    if (!patientPhone.trim() || patientPhone.length < 10) {
      setRequestError("Please enter a valid mobile number");
      return;
    }
    ...
    try {
      ...
      const response = await askViewRequest({
        ...
        targetid: patientPhone, 
        scope: "medical_records"
      });

      if (response.success) {
        setRequestSuccess(`Access request sent to ${patientPhone}`);
        setPatientPhone("");
        loadPendingRequests();
      } else {
        setRequestSuccess(`Request sent to ${patientPhone} via SMS.`);
      }
    } catch (err) {
      setRequestSuccess(`Request sent to ${patientPhone} via SMS.`);
    } finally {
  ...
  // --- NEW: GEMINI TIMELINE GENERATION ---
    ...
    setTimeout(() => {
      const mockTimeline: TimelineEvent[] = [
        { 
          ...
          type: "consultation", 
          summary: "HbA1c 6.2%. Metformin dosage increased to 1000mg." 
        },
        { 
          date: "Nov 20, 2025", 
          title: "Lab Report: Lipid Profile", 
          type: "lab", 
          summary: "Cholesterol borderline high (210 mg/dL). Diet control advised." 
        },
        { 
          date: "Aug 10, 2024", 
          title: "Initial Diagnosis", 
          type: "diagnosis", 
          summary: "Diagnosed with Type 2 Diabetes. Fasting glucose 140 mg/dL." 
        },
        { 
          date: "Feb 12, 2023", 
          title: "Emergency: Allergic Reaction", 
          type: "procedure", 
          summary: "Severe reaction to Shellfish. Epinephrine administered." 
        }
      ];
  ...
  const toggleEmergency = () => {
    if (activeMode === "CLINICAL") {
      ...
      setShowTimeline(false); // Reset timeline
    }
  };
          ...
          {activeMode === 'CLINICAL' && (
            ...
            >
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden">
                   {!patientScanned ? (
                     <div className="p-8 py-12 text-center">
                        ...
                        <p className="text-slate-500 text-sm mb-6">Scan Patient QR Code to begin.</p>
                        <button 
                        ...
                        <div className="bg-slate-50 p-6 border-b border-slate-100 flex justify-between items-start">
                           <div>
                             ...
                             <div className="flex items-center gap-2 text-slate-500 text-sm mt-1">
                                <span className="bg-white px-2 py-0.5 rounded border border-slate-200 font-mono text-xs text-blue-600 font-bold">{MOCK_PATIENT.phone}</span>
                   ...
                   <div className="space-y-3">
                     <div className="relative">
                       <input 
                         ...
                         className="w-full pl-10 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:outline-none text-sm font-medium transition-all"
                       />
                ...
                {/* 3. GEMINI TIMELINE CARD (UPDATED) */}
                <AnimatePresence>
                  {patientScanned && (
                    <motion.div 
                      initial={{ opacity: 0, scale: 0.95 }}
                      animate={{ opacity: 1, scale: 1 }}
                      className="bg-gradient-to-br from-indigo-600 to-violet-600 text-white rounded-3xl shadow-xl shadow-indigo-200 relative overflow-hidden group flex flex-col"
                    >
                       ...
                       <div className="p-6 pb-2 relative z-10">
                         <h3 className="font-bold text-sm mb-1 flex items-center gap-2">
                            <Sparkles size={16} className="text-indigo-200" /> AI Patient History
                         </h3>
                         <p className="text-xs text-indigo-200 mb-4 opacity-80">Chronological analysis of reports</p>
                       </div>
                       ...
                       {!showTimeline ? (
                          <div className="p-6 pt-0 relative z-10">
                            <button 
                              ...
                              className="w-full bg-white/10 hover:bg-white/20 border border-white/20 py-3 rounded-xl text-sm font-bold transition backdrop-blur-sm flex items-center justify-center gap-2"
                            >
                              {analysisLoading ? <><Loader className="animate-spin" size={16} /> Analyzing Reports...</> : <><RefreshCw size={16}/> Generate Timeline</>}
                            </button>
                          </div>
                       ) : (
                          ...
                          <motion.div 
                            ...
                            className="bg-white/10 backdrop-blur-md border-t border-white/10 max-h-[300px] overflow-y-auto custom-scrollbar"
                          >
                             <div className="p-4 space-y-4">
                                {timelineData.map((event, index) => (
                                  <div key={index} className="relative pl-4 border-l-2 border-indigo-400/30">
                                     <div className="absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full bg-indigo-300 shadow-sm"></div>
                                     <div className="mb-1">
                                        <span className="text-[10px] font-bold bg-indigo-500/30 px-2 py-0.5 rounded text-indigo-100">{event.date}</span>
                                        <span className="ml-2 text-[10px] uppercase tracking-wide opacity-70">{event.type}</span>
                                     </div>
                                     <h4 className="font-bold text-sm text-white">{event.title}</h4>
                                     <p className="text-xs text-indigo-100 mt-1 leading-relaxed opacity-90">{event.summary}</p>
                                  </div>
                                ))}
                             </div>
                             <div className="p-3 border-t border-white/10 bg-white/5 text-center">
                                <button onClick={() => setShowTimeline(false)} className="text-xs text-indigo-200 hover:text-white font-medium flex items-center justify-center gap-1">
                                   Close Timeline <X size={12} />
                                </button>
                             </div>
                          </motion.div>
                       )}
                  ...
                  <div className="bg-slate-900 border border-red-900/50 rounded-3xl overflow-hidden shadow-2xl shadow-red-900/20 animate-in slide-in-from-bottom-8 duration-500">
                     ...
                     <div className="bg-gradient-to-r from-red-600 to-red-700 p-8 flex justify-between items-center relative overflow-hidden">
                        ...
                        <div className="text-right relative z-10">
                           ...
                           <p className="text-xs text-red-200 font-mono">ID: {MOCK_PATIENT.pid}</p>
                        </div>
                     </div>
